    ...
    <div className="flex flex-col min-h-screen">
      <header className="h-[70vh] md:h-screen">
        ...
        >
          {[assets.houseImg1, assets.houseImg2, assets.houseImg3].map((img, index) => (
            <div key={index} className="bg-cover bg-center h-[70vh] md:h-screen" style={{ backgroundImage: `url(${img})` }}>
              <div className="bg-black bg-opacity-50 h-full flex flex-col justify-center items-center px-4 text-center">
                <h1 className="text-white text-3xl md:text-5xl font-bold mb-4">Find Your Dream Home</h1>
                <p className="text-white text-base md:text-lg mb-8">Browse through the best properties available.</p>
                <Link to="/propertyList" className="bg-red-500 text-white px-6 py-3 rounded-md text-sm md:text-base">
                  Get Started
                </Link>
              </div>
            </div>
      ...
      <div className="container mx-auto my-8 px-4">
        <section className="my-8">
          <h2 className="text-2xl md:text-3xl font-bold text-center mb-8">Featured Properties</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {Array.isArray(filteredProperties) && filteredProperties.length > 0 ? (
                ...
                <Link key={prop._id} to={`/property/${prop._id}`}>
                  <div className="border rounded-lg overflow-hidden shadow-lg transition-transform transform hover:scale-105">
                    ...
                    <div className="p-4">
                      ...
                      <p className="text-red-500 font-bold text-sm md:text-base">${prop.price}</p>
              ...
              <p className="text-center text-sm md:text-base">No properties available at the moment.</p>
            )}
          </div>
        </section>
      </div>
